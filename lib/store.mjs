...
/* Tokens are always encrypted, but the key should not be something the user has
   to invent before anything works. Falls back to a generated key kept beside
   the store, so connecting requires no configuration at all. */
export async function resolveSecret(env, dataDir){
  if (env.ENCRYPTION_KEY) return env.ENCRYPTION_KEY;
  if (env.APP_PASSWORD) return env.APP_PASSWORD;
  ...
  console.log(`[store] generated an encryption key at ${file}`);
  return generated;
}
