...
/* drive.readonly turns a file NAME into a URL OpusClip can fetch, so a clip can
   be made from "the day one video in Raw videos" without anyone finding the
   link by hand. Read only: locating a file is the whole job, and the write scope
   would also allow moving or deleting one.

   Nothing in the dashboard calls it at present -- /api/drive/find was reached
   only by the voice assistant, which has been removed. The scope is kept
   because it is already granted, and taking it out would mean reconnecting
   Google to get it back.

   It changes the consent screen, so an existing Google connection keeps working
   for mail and calendar and has to be reconnected before a Drive search will
   return anything. That failure says so rather than reporting an empty result. */
const GOOGLE_SCOPES = [
  'openid',
  'email',
...
/* Three scopes because they are three different things: the Data API knows the
   channel's current totals, only the Analytics API breaks anything down by day,
   and only the plain youtube scope can write a title or a description back.

   The write scope is here because the analyst agent's most common finding is a
   packaging problem, and a recommendation you have to go and apply by hand in
   another tab mostly does not get applied. /auth/youtube manages the account's
   own videos and playlists.

   force-ssl is the fourth, and this comment used to say nothing here touched
   it. Comments do. It is the ONLY scope that carries a comment write: readonly
   cannot write at all, and the plain youtube scope covers videos and playlists
   and stops there. Replying to a comment, posting one, editing or deleting
   one's own -- all four are force-ssl, and without it the inbox lists comments
   perfectly well and fails on the first reply.

   Adding it changes the consent screen. An existing YouTube connection keeps
   working for reads and has to be reconnected before a write will succeed, and
   that failure is reported as the reconnect to make rather than as a bare 403. */
...
/* Metrics, ads, comments and messages.

   The five additions are what the Comments & Messages view needs, and each one
   buys exactly one thing:

     pages_read_user_content      read what OTHER people wrote on the Page's
                                  posts. pages_read_engagement covers the
                                  Page's own posts and not the comments on them.
     pages_manage_engagement      reply to a comment, like one, hide one.
     instagram_manage_comments    read Instagram comment TEXT. instagram_basic
                                  returns comments_count and an empty comments
                                  edge, which is indistinguishable from a post
                                  with no comments.
     pages_messaging              the Facebook Page inbox.
     instagram_manage_messages    the Instagram inbox.

   This comment used to say the two messaging permissions would "degrade the
   whole grant" if requested unapproved. That was the wrong worry in this
   deployment: Standard Access grants these to anyone with a role on the app,
   which is who connects here, and a permission the dialog cannot grant simply
   comes back absent from /me/permissions rather than poisoning the rest.
   lib/social-inbox.js reads what was actually granted and hides what was not,
   so an absent permission costs a control rather than a connection.

   The two messaging permissions still need App Review for anyone outside the
   app's own roles, and Instagram messaging additionally needs the messaging
   CAPABILITY on the app -- which is a review, not a scope. That refusal comes
   back as "Application does not have the capability to make this API call" and
...
export const PROVIDERS = {
  ...
  youtube: {
    ...
    refreshKind: 'refresh_token',
    vars: { id: 'GOOGLE_CLIENT_ID', secret: 'GOOGLE_CLIENT_SECRET' },
    setupHint: 'Reuses GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET. Add /oauth/callback/youtube to the same OAuth client\'s redirect URIs, and enable the YouTube Data and YouTube Analytics APIs. Consent now also asks for video editing, which is what lets the analyst agent apply a title or description change you approve.',
    tokenUrl: () => 'https://oauth2.googleapis.com/token',
    authorizeUrl(env, { redirect, state, challenge }){
      return 'https://accounts.google.com/o/oauth2/v2/auth?' + new URLSearchParams({
        client_id: env[this.vars.id],
  ...
  /* Instagram, signed in as Instagram.
     ...
     A SECOND route to the same account, and a deliberate one. The Page route
     above returns 200 and an empty list for this account's DMs -- every other
     explanation ruled out -- which leaves a switch inside the Instagram app
     that only the account owner can reach. This route does not depend on it,
     because the person signing in IS the account owner.

     Its own app id and secret. Instagram Login uses the Instagram App ID from
     the App Dashboard's Instagram product, which is a different number from
     META_APP_ID even inside the same app -- the most likely thing to get wrong
     when setting this up, so providers/instagram.js names that failure.

     No refresh token: like Meta, the durable credential is a long-lived access
     token renewed by exchanging it for a fresh 60 days. */
  instagram_login: {
    label: 'Instagram (direct)',
    blurb: 'Comments and DMs, signed in as the Instagram account itself',
    feeds: ['social'],
    oauth: true,
    refreshKind: 'long_lived_exchange',
    vars: { id: 'INSTAGRAM_APP_ID', secret: 'INSTAGRAM_APP_SECRET' },
    setupHint: 'Set INSTAGRAM_APP_ID and INSTAGRAM_APP_SECRET from the App Dashboard: '
      + 'Instagram product, API setup with Instagram login. They are NOT META_APP_ID '
      + 'and META_APP_SECRET. Add the redirect URI there as well.',
    tokenUrl: () => 'https://api.instagram.com/oauth/access_token',
    authorizeUrl(env, { redirect, state }){
      return instagram.authorizeUrl(env, { redirect, state });
    },
    exchange: instagram.exchange,
    exchangeLongLived: instagram.exchangeLongLived,
    refresh: instagram.refresh,
    identify: instagram.identify,
    discover: instagram.discover
  },
  ...
  /* One named Outlook mailbox, reached as the app rather than as its owner.

     The microsoft entry above is a person signing in and granting their own
     mailbox. This is the tenant granting a named one: client credentials,
     Calendars.ReadWrite as an APPLICATION permission, admin consent. Nobody
     signs in, so nobody has to be at a browser and nothing expires when they
     change their password.

     derived, so the connect sheet does not offer a button for something that
     is configured in the environment. refreshKind 'client_credentials' so the
     ordinary token path mints a fresh one when the hour is up -- there is no
     refresh token in this flow and there does not need to be, because the
     client secret IS the durable credential. */
  microsoft_service: {
    label: 'Outlook calendar (service)',
    blurb: 'One mailbox, read and written as the app',
    feeds: ['calendar'],
    oauth: false,
    derived: true,
    refreshKind: 'client_credentials',
    refresh: env => microsoft.appToken(env),
    setupHint: 'Set MS_TENANT_ID (the real tenant id, not "common"), MS_CLIENT_ID, '
      + 'MS_CLIENT_SECRET and MS_SERVICE_USER, and grant the app Calendars.ReadWrite '
      + 'as an Application permission with admin consent.'
  },

  /* LinkedIn, reached through HeyReach rather than through LinkedIn.

     LinkedIn sells no API for reading a member's own inbox — its Marketing and
     Community Management products cover Pages and ads, and one-to-one messages
     are not among them at any tier. HeyReach already sends those messages and
     holds the replies, so it is the provider here in the same way Meta is the
     provider for a Facebook Page.

     derived, because there is nothing to sign in to: HEYREACH_API_KEY is the
     credential and the sender rows are discovered from it at boot, so the
     connect sheet must not offer a LinkedIn button that leads nowhere.
     refreshKind 'none', because an API key does not expire — a rotated one is
     an environment edit and a redeploy, exactly like GHL's. */
  linkedin: {
    label: 'LinkedIn',
    blurb: 'Conversations from HeyReach',
    feeds: ['social'],
    oauth: false,
    derived: true,
    refreshKind: 'none',
    setupHint: 'Set HEYREACH_API_KEY from HeyReach, Settings, Integrations. The LinkedIn '
      + 'sender accounts on that workspace are discovered from the key; there is no '
      + 'LinkedIn sign-in and nothing to connect here.'
  },
