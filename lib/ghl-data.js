...
/* Three ways in, one SQL:
     leadRows(ids, limit)                     the list
     leadRows(ids, limit, { since })          only rows changed after `since`
     leadRows(ids, 1,     { contactId })      one lead, for a live-update refetch
     leadRows(ids, limit, { offset })         the page after the first

   The offset is an offset and not a keyset cursor, which is a real trade: the
   ordering key is `activity` and it moves whenever anybody messages anybody, so
   a row can cross a page boundary between two requests and be fetched twice or
   missed once. A keyset cursor on a key that mutates is no more stable, only
   harder to read. The caller merges by lead id, which turns the visible half of
   that -- a duplicate row -- into nothing.

   changed_at is the newest of the lead row's own updated_at, its live
   opportunity's updated_at, and the newest message by BOTH sent time and ingest
   time — a backfilled message carries an old ghl_date_added but a new
   created_at, and a delta keyed on sent time alone would never see it. The
   browser keeps the max changed_at it has seen and asks for everything after it,
   so the cursor comes from the server's clock, never the browser's. */
/* search matches a name, an email, a phone number however it was typed, an
   opportunity name or a tag. It is applied before the ORDER BY and the LIMIT, so
   a search looks at every lead in scope rather than only the most recent page --
   which is the entire point of having one. */
export async function leadRows(ids, limit, { since = null, contactId = null, search = null, offset = 0 } = {}){
  const term = String(search || '').trim();
  const like = term ? '%' + term.replace(/[%_\\]/g, m => '\\' + m) + '%' : null;
  /* Phone numbers are stored however GHL had them and typed however the reader
     remembers them. Both sides lose everything that is not a digit before they
     are compared, so "(402) 555-0134" finds 4025550134. */
    ...
    `WITH msg AS (
       SELECT ghl_location_id AS location_id, ghl_contact_id AS contact_id,
        ...
        WHERE ghl_location_id = ANY($1) AND ${NOT_ACTIVITY}
          AND ($4::text IS NULL OR ghl_contact_id = $4)
        GROUP BY 1, 2
     ),
     opp AS (
       SELECT DISTINCT ON (o.ghl_location_id, o.ghl_contact_id)
        ...
        WHERE o.ghl_location_id = ANY($1) AND o.ghl_contact_id IS NOT NULL
          AND ($4::text IS NULL OR o.ghl_contact_id = $4)
     ...
     SELECT * FROM (
       SELECT l.ghl_location_id AS location_id,
              ...
              GREATEST(l.updated_at, o.updated_at, m.last_at, m.last_ingested) AS changed_at
         FROM lead l
        ...
        WHERE l.ghl_location_id = ANY($1)
          AND ($4::text IS NULL OR l.ghl_contact_id = $4)
      ...
      WHERE ($3::timestamptz IS NULL OR changed_at > $3)
        AND ($5::text IS NULL OR
             contact_name ILIKE $5 ESCAPE '\\' OR
             email        ILIKE $5 ESCAPE '\\' OR
             opp_name     ILIKE $5 ESCAPE '\\' OR
             source       ILIKE $5 ESCAPE '\\' OR
             ($6::text IS NOT NULL
              AND regexp_replace(COALESCE(phone, ''), '[^0-9]', '', 'g') LIKE $6) OR
             EXISTS (SELECT 1 FROM unnest(tags) AS t WHERE t ILIKE $5 ESCAPE '\\'))
      ORDER BY activity DESC NULLS LAST
      LIMIT $2 OFFSET $7`,
    [ids, limit, since, contactId, like, digitLike, Math.max(0, Number(offset) || 0)]);
  return rows;
}
...
export async function ingestStatus(){
  ...
  /* The newest run anywhere in the pipeline that completed. A failure older than
     that is one the pipeline has already run past: nothing has retried the
     entity, but everything else has synced since, so it is a stale row rather
     than a live fault. Reporting the two as the same thing is how a red banner
     from last week ends up permanently on screen, at which point it stops being
     read at all. */
  const newestOk = entities
    .filter(e => e.status === 'done' && e.at)
    .reduce((max, e) => Math.max(max, new Date(e.at).getTime()), 0);

  const failed = entities.filter(e => e.status === 'error');
  const current = failed.filter(e =>
    !e.at || !newestOk || new Date(e.at).getTime() >= newestOk);

  return {
    lastRun,
    entities,
    newestOk: newestOk ? new Date(newestOk).toISOString() : null,
    done: entities.filter(e => e.status === 'done').length,
    /* Only the ones the pipeline has not since run past. */
    failing: current.length,
    /* Everything that ended in an error, however old, so the detail line can
       still say what happened without the banner shouting about it. */
    everFailed: failed.length,
    stale: failed.length - current.length,
