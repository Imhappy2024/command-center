...
/* 'leads' is the feed name GHL declares, not the provider name — accountsFor
   filters on what a connection may appear in, and a GHL sub-account feeds the
   Leads view. */
export async function locations(){
  const rows = await accountsFor('leads');
  return rows.map(a => ({ ...a, locationId: a.id.replace(/^ghl:/, '') }));
}

export async function locationIds(){
  return (await locations()).map(a => a.locationId);
}
