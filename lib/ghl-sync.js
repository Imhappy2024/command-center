...
import { accountsFor, getStaticToken, mergeAccountMeta } from './accounts.js';
...
/* ---------------- sending identities ----------------
   ...
   Which numbers and which address a sub-account can send from. Cached onto the
   account row because the composer needs it on every render and the read path is
   not allowed to call GHL — and because it changes when someone buys a number,
   which is roughly never.

   Numbers are enumerable; sending addresses are not. GHL has no endpoint listing
   verified senders, and treats a From matching the location's admin email as
   already verified, so that address is the only one obtainable. The asymmetry is
   real and the UI reflects it rather than inventing an email picker. */
async function syncSenders(locationId, token){
  const senders = { numbers: [], email: null, fetchedAt: new Date().toISOString() };

  try {
    const { numbers } = await limited(locationId, () => ghl.listPhoneNumbers(token, locationId));
    senders.numbers = numbers;
  } catch (err) {
    /* A token without the phone-system scope is a normal setup, not a failure
       worth stopping a backfill over. The composer then says no number is
       available, which is the honest read of "we cannot see one". */
    console.warn(`[ghl:sync] ${locationId} phone numbers unavailable: ${err.message}`);
  }

  try {
    const loc = await limited(locationId, () => ghl.verifyLocation(token, locationId));
    senders.email = loc.email || null;
  } catch (err) {
    console.warn(`[ghl:sync] ${locationId} location details unavailable: ${err.message}`);
  }

  await mergeAccountMeta(`ghl:${locationId}`, { senders }).catch(err =>
    console.error(`[ghl:sync] ${locationId} could not cache senders: ${err.message}`));
...
export async function backfill(locationId, env = process.env, { full = false } = {}){
  ...
  try {
    ...
    /* First, and cheap: the composer is unusable without it, and a first sync
       runs for minutes. */
    await syncSenders(locationId, token);
...
export async function reconcileLocation(locationId, { env = process.env, full = false } = {}){
  ...
  /* Refreshed every pass so buying or releasing a number shows up without a
     reconnect. Two requests an hour per location. */
  await syncSenders(locationId, token);
  ...
  /* Conversations, every pass.

     These used to run only in backfill, which meant messages were fetched exactly
     once per sub-account and never again. With no webhook subscription configured
     that left every thread frozen at whatever the first sync happened to catch —
     and when that first sync ran against the wrong API version it caught nothing,
     so threads were permanently empty with no way to fill them short of a manual
     re-sync.

     A short window on an incremental pass: only threads touched since the last
     run, or a week on a full one. */
  const convoDays = full ? backfillDays(env)
    : Math.max(1, Math.ceil((Date.now() - (since ? Date.parse(since) : 0)) / DAY_MS)) || 7;
  const conversations = await syncConversations(locationId, token,
    { days: Math.min(convoDays, backfillDays(env)) });
  ...
  return {
    contacts: contacts.seen,
    opportunities: opportunities.seen,
