...
export function createApp({ env, publicDir, background = true }){
  ...
  app.locals.background = { intervalMinutes: null, async stop(){} };
  if (background) {
    ...
    app.locals.background = {
      ...
      async stop(){
        /* Both, and both awaited: each holds queries, and the pool closes next. */
        await Promise.all([worker.stop(), reconciler.stop()]);
      }
    };
  }
