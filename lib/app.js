...
import { startReconciler } from './ghl-sync.js';
...
export function createApp({ env, publicDir, background = true }){
  ...
  app.locals.background = { intervalMinutes: null, socialMinutes: null, async stop(){} };
  if (background) {
    const worker = startWorker();
    const reconciler = startReconciler({ env });
    const social = startPoller({ env });
    app.locals.background = {
      intervalMinutes: reconciler.intervalMinutes,
      socialMinutes: social.intervalMinutes,
      async stop(){
        /* All three, all awaited: each holds queries, and the pool closes next. */
        await Promise.all([worker.stop(), reconciler.stop(), social.stop()]);
      }
    };
  }
