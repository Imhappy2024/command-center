...
import { SAFE_TZ } from './timezone.js';
...
import { propertyRoutes, invalidateProperties } from '../routes/properties.js';
...
function shellLoader(publicDir, mode, tz){
  const file = path.join(publicDir, 'index.html');
  /* The clock reads AGENT_TIMEZONE, the same setting every server-rendered
     timestamp in the app already uses. It used to read the BROWSER's zone,
     which meant the header could say one thing while a lead's "2h ago" three
     views away was measured against another — and on a hosted deployment the
     browser is wherever the reader happens to be sitting. */
  const inject = `<script>window.__AUTH_MODE = ${JSON.stringify(mode)};`
    + `window.__BOOT = ${BOOT};`
    + `window.__TZ = ${JSON.stringify(tz || null)};</script>\n</head>`;
  let cache = null;
...
export function createApp({ env, publicDir, background = true }){
  ...
  /* Before everything, so nothing is invisible to it. The auto-updater asks
     this whether it is safe to restart, and a request it never saw is a
     request it would happily kill. */
  app.use(inflight);
  app.use(compression());
  ...
  const auth = makeAuth({
    ...
    /* Injected rather than imported inside session.js, which is deliberately
       free of database imports. Skipped entirely with background:false, the
       flag a test passes to promise that mounting this app opens no
       connection -- and with no lookup a session simply carries no user, which
       is the same thing an APP_PASSWORD session is. */
    loadUser: background ? (id => findById(id)) : null
  });
  ...
  /* No sign-in screen at all when the gate is off. Mounting one would leave a
     form that sets a cookie nothing ever reads, and a Set a password page for
     an account the request is not attached to.

     /api/me is the exception and is mounted either way: the dashboard asks it
     who is signed in, and in open mode the honest answer is "nobody, and that
     is the configuration" rather than a 404 the UI has to guess at. */
  if (auth.isOpen) {
    for (const p of ['/login', '/login.html', '/password']) {
      app.get(p, (req, res) => res.redirect('/'));
    }
    app.get('/api/me', (req, res) => res.json({ authMode: 'open', user: null }));
  } else {
    app.use(authRoutes({ auth, publicDir }));
    app.use('/login.html', (req, res) => res.redirect('/login'));
    app.use('/password.html', (req, res) => res.redirect('/password'));
  }

  app.use(connectRoutes({ env, auth, secret: env.SESSION_SECRET }));
  app.use(mailRoutes({ env, auth }));
  app.use(calendarRoutes({ auth, env }));
  app.use(ghlRoutes({ env, auth, live }));
  app.use(socialRoutes({ env, auth }));
  app.use(inboxRoutes({ auth }));
  /* No auth.require: Meta is the caller and authenticates by signing the body,
     so a session check here would reject every real delivery. */
  app.use(webhookRoutes({ env }));
  app.use(driveRoutes({ auth }));
  app.use(taskRoutes({ env, auth }));
  app.use(propertyRoutes({ env, auth }));
/* After propertyRoutes, which is what installs the invalidator it is handed. */
app.use(propertyDetailRoutes({ env, auth, invalidate: () => invalidateProperties() }));
  ...
  /* SAFE_TZ, not env.AGENT_TIMEZONE: the raw variable can be absent or nonsense,
     and passing it straight through sent the page a null, which fell back to the
     BROWSER's zone — the exact behaviour this was meant to replace. SAFE_TZ is
     the value the rest of the server formats with, already defaulted and already
     proved usable. */
  const loadShell = shellLoader(publicDir, auth.mode, SAFE_TZ);
  ...
  /* The service worker, from the root and in front of auth.

     From the root because a worker's scope cannot be broader than its own
     path, and this one has to control '/'.

     In front of auth for the same reason as the manifest: a 302 to /login in
     answer to /sw.js is not a redirect the browser follows, it is a failed
     registration, and the app is then not installable in any mode that gates
     the page. There is nothing in it worth gating -- it is a caching policy,
     and the one thing it is careful about is never caching /api.

     no-store, because a stale worker is a worker that cannot be replaced. */
  app.get('/sw.js', (req, res) => {
    res.type('text/javascript')
       .set('Cache-Control', 'no-store')
       .set('Service-Worker-Allowed', '/')
       .sendFile(path.join(publicDir, 'sw.js'));
  });
  ...
  /* Everything else is the dashboard shell, which the router draws. auth.require
     rather than auth.valid, so an unknown path gets the same treatment as a
     known one: signed out goes to the sign-in screen, and an account that still
     has to choose a password goes there instead of into the app. */
  app.use((req, res, next) => {
    if (req.path.startsWith('/api/')) return res.status(404).json({ error: 'not found' });
    Promise.resolve(auth.require(req, res, () => shell(req, res).catch(next))).catch(next);
  });
