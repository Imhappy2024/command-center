...
/* Returns the names it actually set, plus the keys it found but left alone
   because they were blank, so the caller can say something true in the boot log.

   A blank value is skipped rather than assigned. `.env.example` is mostly blank
   placeholders, and counting those as "loaded" produced a startup message that
   said it had loaded 38 variables "but not this one" about a key that was right
   there in the file — technically true, actively misleading. */
export function loadEnvFile(file = '.env', env = process.env){
  let text;
  try { text = fs.readFileSync(file, 'utf8'); }
  catch { return Object.assign([], { blank: [], found: false }); }   // the normal hosted case

  const applied = [];
  const blank = [];
  for (const [k, v] of Object.entries(parseEnv(text))) {
    if (v === '') { blank.push(k); continue; }
    /* An injected variable outranks the file. */
    if (env[k] === undefined || env[k] === '') { env[k] = v; applied.push(k); }
  }
  return Object.assign(applied, { blank, found: true });
}
