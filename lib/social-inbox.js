...
const CAP_RULES = {
  ...
  instagram: {
    /* instagram_basic reads the media, instagram_manage_comments reads what is
       written under it. Without the first there is nothing to hang a comment
       on; without the second the comment count comes back and the text does
       not. */
...
export async function sendAttachment(id, { kind, filename, mime, bytes, url = null }){
  ...
  const sent = await meta.sendAttachment(token, {
