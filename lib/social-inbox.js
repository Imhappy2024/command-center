        ...
        if (fb.length) {
          notes.push({
            ...
            error: 'Instagram returned no conversations while this Page’s Messenger inbox '
              + 'returned ' + fb.length + '. That is the signature of Instagram’s own switch, '
              + 'not a permission: in the Instagram app, Settings → Messages and story '
              + 'replies → Connected tools → turn on "Allow access to messages". Until '
              + 'that is on, the API answers with an empty list and no error.',
            needsScope: null
          });
        }
      }
