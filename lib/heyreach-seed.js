...
export async function seedHeyReach(env = process.env){
  ...
  const cleared = await reidentify();
  return { configured: true, seeded, senders: senders.length, cleared };
}

/* A one-time clear, because the key a message is stored under changed.

   The first version hashed the message BODY to make an id, since HeyReach puts
   no id on a message — every one of the 180 in this workspace fell back to it.
   It now hashes the conversation and the timestamp instead, which does not
   move when the same text comes back a character different. Both spellings
   start 'hr:', so the old rows cannot be told from the new ones and would sit
   beside them as duplicates of every message in every thread.

   These tables are a mirror. Dropping the LinkedIn half loses nothing the next
   sync does not fetch again, which is the same reasoning syncInbox already
   applies to an Instagram row a direct sign-in has replaced. Guarded by a row
   in sync_state so it happens once rather than on every boot. */
const REID_KEY = 'heyreach:message-id-scheme';
const REID_VERSION = '2';

async function reidentify(){
  const { rows } = await query(`SELECT cursor FROM sync_state WHERE key = $1`, [REID_KEY]);
  if (rows[0]?.cursor === REID_VERSION) return 0;

  const { rowCount } = await query(
    `DELETE FROM social_items WHERE platform = 'linkedin'`);
  await query(`DELETE FROM social_threads WHERE platform = 'linkedin'`);
  /* The per-account cursors go too, or the next pass asks for what changed
     since a sync whose results were just deleted. */
  await query(
    `DELETE FROM sync_state WHERE key LIKE 'inbox:linkedin:%'`);
  await query(
    `INSERT INTO sync_state (key, cursor, last_run) VALUES ($1, $2, now())
     ON CONFLICT (key) DO UPDATE SET cursor = EXCLUDED.cursor, last_run = now()`,
    [REID_KEY, REID_VERSION]);

  if (rowCount) {
    console.log(`[heyreach] cleared ${rowCount} message(s) stored under the old id scheme; `
