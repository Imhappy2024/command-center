...
const publicRow = r => ({
  ...
  /* Which mailbox an app-only Graph credential should act on. Null for every
     delegated account, where the token already names its own owner and the
     path is /me. Not a secret — it is an address the calendar already shows. */
  mailbox: r.meta?.mailbox || null
});

/* An asset that arrived with a grant but does not belong in this dashboard --
   a personal ad account that came along with the business ones, say. Deleting
   the row does not hold: the next Meta reconnect rediscovers it and puts it
   straight back. So the row stays as a tombstone and every listing skips it,
   which is also what stops the poller, the social inbox and the webhook
   receiver from touching it -- they all read through accountsFor(). */
const isExcluded = r => r.meta?.excluded === true;
...
export async function listAll(){
  ...
  return rows.filter(r => !isExcluded(r)).map(publicRow);
}
...
export async function upsertOAuth({ provider, uid, email, label, color, refreshToken, accessToken, expiresAt, scope, meta = {} }){
  ...
  await query(
    `INSERT INTO accounts
     ...
     /* An excluded row is left exactly as it is. Without this the next
        reconnect would hand it a fresh token and set status back to 'ok',
        and the asset the owner removed would be live again. */
     WHERE COALESCE(accounts.meta->>'excluded', 'false') <> 'true'`,
...
/* Kept, not deleted, and invisible either way. The distinction matters only
   for a discovered asset, where a delete lasts until the next reconnect. */
export async function excludeAccount(id){
  const { rowCount } = await query(
    `UPDATE accounts
        SET meta = COALESCE(meta, '{}'::jsonb) || '{"excluded":true}'::jsonb
      WHERE id = $1`,
    [id]
  );
  return rowCount > 0;
